...
class ResNet18_3D_7stream_LSTM(nn.Module):
    def __init__(self, in_channels, n_classes, pretrained=True, no_cuda=False):
        super(ResNet18_3D_7stream_LSTM, self).__init__()
        ...
        self.backbone4 = resnet.resnet18(in_channels=in_channels, sample_input_W=1, sample_input_H=1, sample_input_D=1,
                                         shortcut_type='A', no_cuda=no_cuda, num_seg_classes=2)
        self.backbone5 = resnet.resnet18(in_channels=in_channels, sample_input_W=1, sample_input_H=1, sample_input_D=1,
                                         shortcut_type='A', no_cuda=no_cuda, num_seg_classes=2)
        self.backbone6 = resnet.resnet18(in_channels=in_channels, sample_input_W=1, sample_input_H=1, sample_input_D=1,
                                         shortcut_type='A', no_cuda=no_cuda, num_seg_classes=2)
        self.backbone7 = resnet.resnet18(in_channels=in_channels, sample_input_W=1, sample_input_H=1, sample_input_D=1,
                                         shortcut_type='A', no_cuda=no_cuda, num_seg_classes=2)
        ...
        if pretrained:
            ...
            self.backbone5.load_state_dict(net_dict)
            self.backbone6.load_state_dict(net_dict)
            self.backbone7.load_state_dict(net_dict)
        ...
        self.mp = nn.MaxPool1d(7, stride=1)
    ...
    def forward(self, x1, x2, x3, x4, x5, x6, x7):
        ...
        x5 = self.feature_extraction(x5, self.backbone5)
        x6 = self.feature_extraction(x6, self.backbone6)
        x7 = self.feature_extraction(x7, self.backbone7)
        ...
        x = torch.cat((x1.view(x1.size(0), 1, -1), x2.view(x1.size(0), 1, -1), x3.view(x1.size(0), 1, -1), x4.view(x1.size(0), 1, -1), x5.view(x1.size(0), 1, -1), x6.view(x1.size(0), 1, -1), x7.view(x1.size(0), 1, -1)), dim=1)
        x, _ = self.LSTM(x)
